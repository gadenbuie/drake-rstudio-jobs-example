...
```{r eval=TRUE, echo=FALSE, results='asis'}
log_file("Qo5K41KcCy", 1)
```

### Step 2

```{r eval=TRUE, echo=FALSE, results='asis'}
log_file("FxzLUU8vWM", 2)
```
...
```{r eval=TRUE, echo=FALSE, results='asis'}
log_file("b1McGwZTEk", 1)
```

### Step 2

This time `data` is remade because the dependencies changed when `import_data()` was sourced into a different environment.
Oddly, the target `regression1_small` appears in `outdated(config)` but apparently isn't out of date once the `data` is remade.

```{r eval=TRUE, echo=FALSE, results='asis'}
log_file("RbhF9FW4q2", 1)
```

### Step 3

```{r eval=TRUE, echo=FALSE, results='asis'}
log_file("pwNOjiulGL", 2)
```

## Results

Sourcing inside scripts launched as background RStudio jobs might be problematic.
